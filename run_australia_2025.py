...
#BASEDIR = '/mnt/brunost/song/ssmtkent/'
...
TRACE_YMIN = 130  # Lower cutoff for order trace (central column pix)
...
def run_inner(opts, logger):
    ...
    # Note that we set image_class=Image and not QHY, as we don't want to apply windowing twice
    ...
    for mode, calibration_set in calibs.items():
        calibration_set.trace_orders(ymin=TRACE_YMIN, ymax=TRACE_YMAX, min_width=TRACE_MIN_WIDTH, target_nord=TRACE_NORD)
